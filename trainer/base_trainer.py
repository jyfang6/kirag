...
class BaseTrainer(nn.Module):
    ...
    def disable_dropout_in_model(self, module: nn.Module):
    ...
    def log(self, name, value):
        ...
        if caller_function in ["training_step", "training_step_start", "training_step_end", "train"]:
            if self.use_ddp and not torch.is_tensor(value):
    ...
    def get_dataloader(self, dataset: Dataset, batch_size: int, shuffle: bool, collate_fn=None, drop_last: bool=False) -> DataLoader:

        if self.use_ddp:
            sampler = DistributedSampler(dataset, num_replicas=self.world_size, rank=self.local_rank, drop_last=drop_last, shuffle=shuffle)
    ...
    def unwrap_ddp_model(self, model):
            ...
            for batch_idx, batch in enumerate(train_dataloader):
                ...
                if batch_idx % self.accumulate_grad_batches == 0: 
                    ...
                    for log_n, log_t in self.log_tensor.items():
                        log_t = log_t / self.world_size
                        log_t = self.sum_main(log_t, main_process, self.use_ddp)
                        self.log_variable_per_update_step[log_n] += (log_t.item() if torch.is_tensor(log_t) else log_t)
                    self.log_tensor = {}
    ...
    def evaluate(self, dataloader, main_process=True):
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            assert isinstance(batch_scores, (tuple, list, np.ndarray)) 
    ...
    def get_scheduler(self, optimizer):
        ...
        if "linear" in self.scheduler_name:
            warmup_steps = self.warmup_steps * self.max_training_steps \
                if isinstance(self.warmup_steps, float) and self.warmup_steps < 1.0 \
    ...
    def get_optimizer_grouped_parameters(self, model, no_decay_params=[]):
        assert isinstance(no_decay_params, (tuple, list)) 
