...
from retriever.e5_mistral import get_e5_mistral_embeddings_for_query, get_e5_mistral_embeddings_for_document
...
from retriever.rerankers import BaseReranker, BGEKGChainReranker
from data.kg_adaptive_collators import E5Collator, BGECollator, ContrieverCollator
from data.reranker_collators import BGEKGChainRerankerCollator
...
class QuerReformulator(nn.Module):

    def __init__(self, reformulate_type: str="concat", convert_triple_to_text: bool=False, **kwargs):
        super().__init__()
        self.reformulate_type = reformulate_type 
        self.convert_triple_to_text = convert_triple_to_text
        self.kwargs = kwargs

    def reformulate(self, question: str, reasoning_chain: List[str], **kwargs):

        """
        Input:
            question: str
            reasoning_chain: ["<xxx; xxx; xxx>", "<xxx; xxx; xxx>"]
        Output:
            query: str 
        """
        if self.convert_triple_to_text:
            reasoning_chain = convert_triples_to_sentences(reasoning_chain)

        if self.reformulate_type == "concat":
            if len(reasoning_chain) > 0:
                query = "{}\nknowledge triples: {}.".format(question, ". ".join(reasoning_chain))
                # TODO: 我调换了 knowledge triples和question的位置，如果没用的话记得把它换回来
                # query = "knowledge triples: {}\nquestion: {}".format(". ".join(reasoning_chain), question)
                # TODO: 只拼接question和最后一个triples
                # query = "{}\nknowledge triples: {}".format(question, reasoning_chain[-1])
            else:
                query = question
        else:
            raise NotImplementedError(f"Reformulate Type \"{self.reformulate_type}\" is not implemented!")
    
        return query 

    def forward(self, question: str, reasoning_chains: List[List[str]], **kwargs):

        """
        Input:
            question: str
            reasoning_chains: [ ["<xxx; xxx; xxx>", "<xxx; xxx; xxx>"] ]
        Output:
            queries: [str]
        """
        if reasoning_chains is None or (reasoning_chains is not None and len(reasoning_chains) == 0):
            reasoning_chains = [[]]
        
        queries = [self.reformulate(question=question, reasoning_chain=reasoning_chain, **kwargs) for reasoning_chain in reasoning_chains]
        return queries

class LLMQuerReformulator(nn.Module):

    def __init__(
        self, 
        tokenizer: AutoTokenizer, 
        reformulator: AutoModelForCausalLM, 
        max_length: int=4096, 
        max_new_tokens: int=128, 
        convert_triple_to_text: bool=True,
        batch_size: int=4, 
        **kwargs 
    ):
        super().__init__()
        ...
        self.device = self.reformulator.device 
        ...
        # self.task_instruction = "You are given a question and a list of knowledge triples. Your task is to reformulate the question by incorporating relevant knowledge triples to form a new query for retrieving additional documents. "\
        #     "Ignore the triples that are not helpful in answering the question. The goal is to create a comprehensive and precise query that leverages useful information from the knowledge triples. "\
        #         "Please output in this order: (1) which triples are relevant and reasons; (2) the reformulated question using the format: \"The reformulated question is:\"."

        separation = "(separated by period)" if self.convert_triple_to_text else "(in the form of <head entity; relationship; tail entity> that describes the relationship between head entity and tail entity)"
        self.task_instruction = f"You are given a question and a list of knowledge triples {separation}. Your task is to generate a follow-up question based on the initial question and the provided knowledge triples. "\
            "The follow-up question should aim to retrieve additional information necessary for answering the initial question. Let's think step-by-step:\n"\
                ...
                # "If existing triples are sufficient to answer the question, output \"Existing triples are sufficient to answer the question, no additional retrieval is needed.\""\
                #     "Otherwise, you MUST use the format \"So the follow-up question is:\" to output the follow-up question."
        
        # self.task_instruction += "Step-by-Step Instructions:\n\n"\
        #     "1. Understand the question: Analyze the given question to identify its key components (entities, relationships, and desired information).\n"\
        #     "2. Evaluate knowledge triples: For each knowledge triple, determine its relevance by checking if it adds meaningful context or information to the question.\n"\
        #     "3. Incorporate relevant triples: Integrate the relevant triples into the question, ensuring that the resulting query remains coherent and enhances the original question.\n"\
        #     "4. Formulate the query: Combine the original question with the relevant triples to form a detailed and precise query.\n"\
        #     "5. Review and refine: Ensure the reformulated query is clear and focused on retrieving relevant documents."
        
        self.examplars = [
            {
                "question": "Where was the director of film 4.3.2.1. born?",
                "triples": [], 
                "output": "Analyses: (1) Identify the director of the film \"4.3.2.1.\". (2) Determine the birthplace of this director.\nRelevance: No knowledge triples are provided, so no relevance can be assessed.\nUnsolved steps: (1) Identify the director of the film \"4.3.2.1.\". (2) Determine the birthplace of this director.\nSo the follow-up question is: Who is the director of the film \"4.3.2.1.\" and where was this director born?", 
            },
            # single-hop example
            {
            ...
            # four-hop example 
            {
                "question": "Which film has the director born later, White Pebbles or For The Sake Of My Intemperate Youth?", 
                "triples": ["<White Pebbles; director; Richard Thorpe>", "<For The Sake Of My Intemperate Youth; director; Arne Mattsson>", "<Richard Thorpe; date of birth; February 24, 1896>"], 
                "output": "Analyses: (1) Identify the directors of the films \"White Pebbles\" and \"For The Sake Of My Intemperate Youth.\". (2) Determine the birth dates of these directors. (3) Compare the birth dates to see which director was born later.\nRelevance: The first triple indicates the director of White Pebbles is Richard Thorpe, addressing the first step. The second triple indicates that the director of For the Sake of My Intemperate Youth is Arne Mattsson, addressing the first step. The third triple indicates that the birthday of Richard Thorpe is February 24, 1896, addressing the second step. However, there is no information about Arne Mattsson's birth date.\nUnsolved steps: (1) Determine the birth date of Arne Mattsson. (2) Compare the birth dates of Richard Thorpe and Arne Mattsson to see which director was born later.\nSo the follow-up question is: when is the birthday of Arne Mattsson?", 
            },
            # comparison example
            {
            ...
            # irrelevant example 
            # {
            #     "question": "Between Love Among Freaks and Sum 41 which one was originally called Kaspir?", 
            #     "triples": ["<Love Among Freaks; founded in; New Jersey, USA area>"],
            #     # "output": "There are no relevant knowledge triples. The reformulated question is: Who distributed a movie whose cast includes an actor who acted in \"Him\"?",
            #     "output": "Analyses: (1) Identify the original name of Love Among Freaks. (2) Identify the original name of Sum 41. (3) Determine if either Love Among Freaks or Sum 41 was originally called Kaspir.\nRelevance: The provided triple provides information about where Love Among Freaks was founded, but it does not provide information about its original name.\nUnsolved steps: (1) Identify the original name of Love Among Freaks. (2) Identify the original name of Sum 41. (3) Determine if either Love Among Freaks or Sum 41 was originally called Kaspir.\nSo the follow-up question is: What was the original name of Sum 41? And what was the original name of Love Among Freaks?",
            # },
            # {
            #     "question": "What lake borders the city where the 308th Bombardment Wing was last stationed?",
            #     "triples": ["<736th Bombardment Squadron; last station; Columbus Air Force Base, Mississippi>", "<308th Bombardment Wing (U.S. Army Air Forces); last location; Nagoya, Japan>"],
            #     "output": "Analyses: (1) Identify the city where the 308th Bombardment Wing was last stationed. (2) Determine the lake that borders that city.\nRelevance: The first triple provides information about the 736th Bombardment Squadron and is not relevant to the 308th Bombardment Wing. The second triple identifies Nagoya, Japan as the last location of the 308th Bombardment Wing, addressing the first step.\nUnsolved steps: Determine the lake that borders Nagoya, Japan.\nSo the follow-up question is: What lake borders Nagoya, Japan?"  
            # },
            {
                "question": "Justin Shenkarow voices Harold Berman, a character from a television series that aired on what station?", 
                "triples": ["<Justin Shenkarow; occupation; actor, voice actor, producer, writer, director>", "<Justin Shenkarow; notable roles; Matthew Brock in \"Picket Fences\">"],
                "output": "Analyses: (1) Identify the television series in which Justin Shenkarow voices the character Harold Berman. (2) Determine the television station that aired this series.\nRelevance: The first triple provides information about Justin Shenkarow's occupations but does not provide information about the television series or station. The second triple provides information about another notable role of Justin Shenkarow but does not mention the television series \"Hey Arnold!\" where he voices Harold Berman or the station that aired it.\nUnsolved steps: (1) Identify the television series in which Justin Shenkarow voices Harold Berman. (2) Determine the television station that aired this series.\nSo the follow-up question is: Which television series features Justin Shenkarow voicing the character Harold Berman, and what station aired this series?"
            },
            {
                ...
                "triples": ["<Jones Soda; type of soft drink; carbonated soft drink>"],
                "output": "Analyses: (1) Identify the soft drink that the ICP (Insane Clown Posse) traditionally sprays the audience with. (2) Identify the company that produces this soft drink. (3) Determine the location of the headquarters of that company.\nRelevance: The first triple provides information about Jones Soda being a carbonated soft drink, but does not establish if it is the soft drink used by ICP, nor does it provide the headquarters location.\nUnsolved steps: (1) Identify the soft drink that ICP traditionally sprays the audience with. (2) Confirm the company that produces this soft drink. (3) Determine the location of the headquarters of that company.\nSo the follow-up question is: What soft drink does ICP traditionally spray the audience with? Which company produces this soft drink, and where is the headquarters of that company located?",
            },            
        ]
    ...
    def get_instruction(self):
        ...
        examplars_texts_list = [] 
        format = "Question: {q}\nKnowledge Triples: {t}\nOutput: {r}"
        for example in self.examplars:
            if self.convert_triple_to_text:
                triples_texts_list = [convert_triples_to_sentences(triple) for triple in example["triples"]]
            else:
                triples_texts_list = example["triples"]
            example_text = format.format(
                q=example["question"],
                t=". ".join(triples_texts_list),
                r=example["output"]
            )
            examplars_texts_list.append(example_text)
        instruction += "\n\n".join(examplars_texts_list)

        return instruction 
    
    def get_reformulator_inputs(self, question: str, reasoning_chains: List[List[str]]):
        inputs = [] 
        format = "Question: {q}\nKnowledge Triples: {t}\nOutput: "
        for chain in reasoning_chains:
            if self.convert_triple_to_text:
                triples_texts_list = convert_triples_to_sentences(chain)
            else:
                triples_texts_list = chain 
            inputs.append(format.format(q=question, t=". ".join(triples_texts_list)))
        return inputs
    
    def get_reformulator_prompts_chat_format(self, instructions: List[List[str]], inputs: List[List[str]]):
        """
        Input: 
        ...
        for instruction, input in zip(instructions, inputs):
            if isinstance(self.reformulator, (LlamaForCausalLM, Qwen2ForCausalLM)):
                prompts.append(
                    [
    ...
    def reformulator_generate(self, inputs: Dict[str, Tensor]):
        generated_texts_list = [] 
        for i in range((len(inputs)-1)//self.batch_size+1):
            batch_inputs = {k: v[i*self.batch_size:(i+1)*self.batch_size] for k, v in inputs.items()}
            batch_inputs = to_device(batch_inputs, self.device)
            batch_generated_ids = self.reformulator.generate(**batch_inputs, max_new_tokens=self.max_new_tokens, temperature=1.0, do_sample=False)
            batch_generated_ids = batch_generated_ids[:, batch_inputs["input_ids"].shape[-1]:]
            batch_generated_texts = self.tokenizer.batch_decode(batch_generated_ids, skip_special_tokens=True)
            generated_texts_list.extend(batch_generated_texts)
        return generated_texts_list
    
    def parse_reformulator_output(self, question: str, generated_texts: List[str], reformulate_flags: List[bool], chains_finihsed: List[bool]):
        
        reformulated_questions = []
        for flag, generated_text, finished in zip(reformulate_flags, generated_texts, chains_finihsed):
            if not flag:
                # 不需要reformulate的两种情况: (1) chain为0, 直接返回原答案 (2) chain已经Finished, 返回None 
                if not finished:
                    reformulated_questions.append(question)
                else:
                    reformulated_questions.append(None)
            else:
                # split_pieces = generated_text.split("reformulated question is")
                # 判断不需要的时候直接返回None
                if "no additional follow-up question is needed" in generated_text.lower():
                    reformulated_questions.append(None)
                    continue

                split_pieces = generated_text.split("follow-up question is")
                # split_pieces = generated_text.split("Unsolved steps")
                if len(split_pieces) == 2:
                    reformulated_question = re.sub(r'^\W+|\W+$', "", split_pieces[1]).strip()
                    # reformulated_question = split_pieces[1].strip()
                    if len(reformulated_question) > 0:
                        reformulated_questions.append(reformulated_question)
                        # reformulated_questions.append("Unsolved steps" + reformulated_question)
                    else:
                        reformulated_questions.append(question)
                else:
                    reformulated_questions.append(question)
    ...
    def reformulate(self, question: str, reasoning_chain: List[str], **kwargs):
        ...
        if len(reasoning_chain) == 0:
            return question
        reformulated_question = self.forward(question=question, reasoning_chains=[reasoning_chain], **kwargs)[0]
        return reformulated_question
    
    def forward(self, question: str, reasoning_chains: List[List[str]], chains_finished: Optional[List[bool]]=None, **kwargs):
        ...
        Input:
            ...
            reasoning_chains: [ ["<xxx; xxx; xxx>", "<xxx; xxx; xxx>"] ]
        Output:
            queries: [str] str might be None
        """

        if reasoning_chains is None or (reasoning_chains is not None and len(reasoning_chains) == 0):
            reasoning_chains = [[]]

        # chains_finished = kwargs.pop("chains_finished", [False]*len(reasoning_chains))
        # reformulate_flags = [len(reasoning_chain)>0 for reasoning_chain in reasoning_chains]
        if chains_finished is None:
            chains_finished = [False] * len(reasoning_chains)
        reformulate_flags = [] # 表示是否需要对question进行reformulate 
        for reasoning_chain, chain_finished in zip(reasoning_chains, chains_finished):
            if len(reasoning_chain) == 0:
                reformulate_flags.append(False) # 没有triple的时候只用原始的question进行检索
                continue
            if chain_finished:
                reformulate_flags.append(False)
            else:
                reformulate_flags.append(True)
        
        if any(reformulate_flags):
            instruction = self.get_instruction()
            instructions = [instruction] * len(reasoning_chains)
            inputs = self.get_reformulator_inputs(question, reasoning_chains)
            prompts = self.get_reformulator_prompts_chat_format(instructions, inputs)
            reformulator_inputs = self.tokenizer_encode_chat_format(prompts)
            generated_texts = self.reformulator_generate(reformulator_inputs)
            reformulated_questions = self.parse_reformulator_output(question, generated_texts, reformulate_flags, chains_finished)
            # from pdb import set_trace; set_trace()
        else:
            reformulated_questions = [question] * len(reasoning_chains)
            # generated_texts = "" # TODO: DELETE 
        
        return reformulated_questions

class OpenAIGPTQueryReformulator(OpenAIGPTGenerator):

    def __init__(self, max_new_tokens: int=256, **kwargs):
        ...
        for example in self.examplars:
            ...
            self.chat_history.append({"user": user})
            self.chat_history.append({"system": example["output"]})
        ...
        if any(reformulate_flags):
            ...
            for reasoning_chain in reasoning_chains:
                ...
                output = self.generate(
                    instruction=self.task_instruction, 
                    user_inputs=self.chat_history + [{"user": input_content}],
...
class TripleSelector(nn.Module):
    ...
    def __init__(
        ...
        batch_size: int=4, # LLM的batch_size 
        query_reformulator=None, # None时会默认初始化一个简单拼接的query_reformulator 
        ...
        **kwargs, 
        ...
        if self.use_cot:
            adaptive_examplars = False # 只标注了前5个demonstration
            num_examplars = min(num_examplars, 5)

        self.tokenizer = tokenizer
        self.selector = selector
        assert isinstance(selector, (LlamaForCausalLM, Qwen2ForCausalLM)) # currently only support using LLaMA3 or Gemma2 as the generator
...
class TextSelector(TripleSelector):

    """
    构造以document或者sentence为基础的reasoning chain
    """
    
    def get_candidate_texts_from_documents(self, documents: List[Dict[str, Union[str, List]]], reasoning_chains_type: str):
        """
        Input:
            documents: [{"id": str, "title": str, "text": str / "sentence": str}]
        Output:
            candidates: [{"title": str, "text": str, "reference": [str(document id), None/int(sentence idx)]}]
        """
        candidates = [] 
        for doc in documents:
            doc_id = doc["id"]
            title = doc["title"]
            if reasoning_chains_type == "document":
                text = doc["text"] if "text" in doc else " ".join(doc["sentences"])
                candidates.append({"title": title, "text": text, "reference": [doc_id, None]}) 
            elif reasoning_chains_type == "sentence":
                sentences = doc["sentences"] if "sentences" in doc else sent_tokenize(doc["text"])
                for sentence_idx, sentence in enumerate(sentences):
                    candidates.append({"title": title, "text": sentence, "reference": [doc_id, sentence_idx]})
            else:
                raise NotImplementedError(f"\"{reasoning_chains_type}\" is not a supported reasoning chains type!")
     
        return candidates 

    def convert_examplar_reasoning_chain_to_texts(self, chain: str):
        """
        Input: 
            chain: "<xx1; xx2; xx3>, <xx4; xx5, xx6>" or "B. <xxx; xxx; xxx>" or "A. no need for additional triples"
        Output:
            xxx1 xx2 xx3. xx4 xx5 xx6 
        """
        pattern = r"<(.*?)>"
        matches = re.findall(pattern, chain)
        texts = [string.replace(";", "") for string in matches]
        return ". ".join(texts)

    def get_selector_inputs(
        self, 
        question: str, 
        existing_texts: List[List[str]], 
        candidate_texts: List[List[str]], 
        ranked_examplars_indices: List[int],
        reasoning_chains_type: str, 
    ):
        ...
        if reasoning_chains_type == "document":
            task_instruction = "Select the next document that extends an existing set of documents to form a coherent reasoning path capable of answering a specified question. " \
                "If the current reasoning path is sufficient to answer the question, simply output 0. Please only output the choice for the next document."
        elif reasoning_chains_type == "sentence":
            task_instruction = "Select the next sentence that extends an existing set of sentences to form a coherent reasoning path capable of answering a specified question. " \
                "If the current reasoning path is sufficient to answer the question, simply output 0. Please only output the choice for the next sentence."
        else:
            raise NotImplementedError(f"\"{reasoning_chains_type}\" is not a supported reasoning chains type!")

        instructions, inputs = [], [] 
        for texts, candidates in zip(existing_texts, candidate_texts):
            instruction = task_instruction
            examplars = [] 
            hop = len(texts)
            if self.num_examplars > 0:
                instruction +=  "\n\nThe followings are some examples of coherent reasoning paths capable of answering the specified question " \
                    f"and how the {hop+1}-th {reasoning_chains_type}s in these paths are selected:\n\n"
                for index in ranked_examplars_indices:
                    reasoning_chain_examplar = self.reasoning_chain_examplars[index]
                    triple_selection_examplar = self.triple_selection_examplars[index]
                    if len(triple_selection_examplar) < hop + 1:
                        continue
                    examplar = "coherent reasoning path: {}\nquestion: {}\n".format(
                        self.convert_examplar_reasoning_chain_to_texts(reasoning_chain_examplar["chains"]), 
                        reasoning_chain_examplar["question"]
                    )
                    examplar += "The {}-th {} in the reasoning path is selected as:\n".format(hop+1, reasoning_chains_type)
                    one_step_triple_selection = triple_selection_examplar[hop]
                    examplar += "existing {}s: {}\nquestion: {}\ncandidate {}s:\n{}\nthe next possible {} is:{}\n".format(
                        reasoning_chains_type, 
                        ". ".join(
                            [
                                self.convert_examplar_reasoning_chain_to_texts(triple_text) \
                                    for triple_text in one_step_triple_selection["triples"]
                            ]
                        ), 
                        one_step_triple_selection["question"], 
                        ...
                        "\n".join(
                            self.convert_candidate_triples_to_choices(
                                [
                                    self.convert_examplar_reasoning_chain_to_texts(ct) \
                                        for ct in one_step_triple_selection["candidate_triples"][1:]
                                ]
                            )
                        ), 
                        reasoning_chains_type, 
                        self.convert_examplar_answer_to_numbers(one_step_triple_selection["answer"])
                    )
                    examplars.append(examplar)
                    if len(examplars) >= self.num_examplars:
                        break

            input_text = "The {}-th {} in the reasoning path is selected as:\nexisting {}s: {}\nquestion: {}\ncandidate {}s:\n{}\nthe next possible {} is:".format(
                hop + 1, reasoning_chains_type, reasoning_chains_type, 
                " ".join(texts), question, reasoning_chains_type, 
                "\n".join(self.convert_candidate_triples_to_choices(candidates)), reasoning_chains_type
            )
    ...
    def forward(
        ...
        documents: List[Dict[str, Union[str, List]]],
        existing_reasoning_chains: Optional[List[Dict[str, Union[List, float, bool]]]]=None,
        reasoning_chains_type: Optional[str]="document", # "document" or "sentence" 
        num_beams: int=5, 
        max_num_chains: Optional[int]=None, # 设置为None返回所有可能的chains
        min_triple_prob: Optional[float]=1e-4, 
    ):
        ...
        Input:
            ...
            existing_reasoning_chains: [
                "triples": [
                    {"title": str, "text": "document or sentence", "reference": [str(document id), None/int(sentence idx)]}
                ],
                "score": float, 
                "finished": bool
            ],
            reasoning_chains_type: "document" or "sentence" 
        Output:
            new_reasoning_chains: the same format as existing_reasoning_chains
        """
        assert reasoning_chains_type in ["document", "sentence"] # reasoning_chains_type must be chosen from ["document", "sentence"]
        if existing_reasoning_chains is not None and len(existing_reasoning_chains) == 0:
            existing_reasoning_chains = None
        chains, chains_scores, chains_finished = self.parse_reasoning_chains(existing_reasoning_chains)
        if existing_reasoning_chains is not None and np.sum(chains_finished) == len(existing_reasoning_chains):
        ...
        # {"title": str, "text": str, "reference": [doc_id, senence_idx/None]}
        all_candidates = self.get_candidate_texts_from_documents(documents, reasoning_chains_type)

        if self.use_triple_filter:
            candidate_indices, _ = self.filter_candidate_triples(question, chains, all_candidates, self.num_candidate_triples)
        else:
            candidate_indices = [list(range(min(len(all_candidates), self.maximum_possible_choices)))for _ in range(len(chains))]
        
        candidate_indices_wo_duplicates, *others = self.filter_duplicate_triples(chains, all_candidates, candidate_indices)
        maximum_num_candidates = max([len(candidates) for candidates in candidate_indices_wo_duplicates])

        existing_texts = self.get_reasoning_chains_texts(chains)
        candidates_wo_duplicates = [
            [self.get_triple_text(all_candidates[idx]) for idx in indices] \
                for indices in candidate_indices_wo_duplicates
        ]
        instructions, inputs = self.get_selector_inputs(
            question, 
            existing_texts=existing_texts, 
            candidate_texts=candidates_wo_duplicates, 
            ranked_examplars_indices=ranked_examplars_indices,
            reasoning_chains_type=reasoning_chains_type
        )
        prompts = self.get_selector_prompts_chat_format(instructions, inputs)
        selector_inputs = self.tokenizer_encode_chat_format(prompts)
        generated_token_ids, generated_token_logits = self.selector_generate(selector_inputs)
        ...
        option_tokens_list, option_tokens_probs = self.get_option_tokens_probs(
            generated_token_ids, generated_token_logits, maximum_num_candidates + 1 
        )
        ...
        for i in range(len(chains)):
            candidate_indices_for_current_chain = candidate_indices_wo_duplicates[i]
            # 如果当前路径已经结束或者没有任何的candidates的话，直接复制当前路径并结束
            if chains_finished[i] or len(candidate_indices_for_current_chain) == 0:
                new_chains.append(chains[i])
                new_chains_scores.append(chains_scores[i])
                new_chains_finished.append(True)
                continue
            ...
            for b in range(min(option_tokens_probs.shape[1], num_beams)):
                ...
                current_choice = option_tokens_list[topk_options_indices[i, b].item()]
                # 如果模型给的选项超过了当前可能的选项的时候跳过, 如果candidate_triples一共有两个，那么option就是["0", "1", "2"] 
                if current_choice != '0' and int(current_choice) > len(candidate_indices_for_current_chain):
                # if current_choice != 'A' and self.choices_to_indices[current_choice] >= len(candidate_triples_indices_for_current_chain):
                    continue
                new_chains_scores.append(chains_scores[i]*topk_options_probs[i, b].item())
                if current_choice == "0":
                    ...
                    # -1是因为当choice为1的时候，实际上是candidate_triples_indices_for_current_chain中index为0的triple
                    new_chains.append(
                        chains[i]+[all_candidates[candidate_indices_for_current_chain[int(current_choice)-1]]]
                    )
                    # new_chains.append(
                    #     chains[i]+[all_triples[candidate_triples_indices_for_current_chain[self.choices_to_indices[current_choice]]]]
                    # )
        ...
        results = []
        for chain, chain_score, chain_finished in zip(chains, chains_scores, chains_finished):
            results.append({"triples": chain, "score": chain_score, "finished": chain_finished})
        
        return results
    

class T5TripleSelector(TripleSelector):
    ...
    def __init__(
        ...
        **kwargs, 
        ...
        if self.query_reformulator is None:
            self.query_reformulator = QuerReformulator(reformulate_type="concat", convert_triple_to_text=False)
        self.use_title_in_triples = use_title_in_triples
        self.task_instruction="Select the next knowledge triple to form a coherent reasoning path to answer the question. Please only output the option for the next knowledge triple. "
    ...
    def get_selector_inputs(
        ...
        existing_triples: List[List[str]], 
        candidate_triples: List[List[str]],
        ranked_examplars_indices: List[int]=None,
    ):
        """
        Input:
            question: str
        ...
        Output:
            ...
            inputs: [str, ...]
        """
        def prompt(q, ets, cts):
            choices = ["no need for additional knowledge triples"] + cts
            choices = ["{}. {}".format(str(i+1), triple) for i, triple in enumerate(choices)]
            candidates_text = "\n".join(choices)
            if len(ets) == 0:
                return f"Question: {q}\nCandidate Triples:\n{candidates_text}\nSelected Triple Option: "
            else:
                existing_triples_text = ". ".join(ets)
                return f"Question: {q}\nknowledge triples: {existing_triples_text}.\nCandidate Triples:\n{candidates_text}\nSelected Triple Option: "
        ...
        for triples, candidates in zip(existing_triples, candidate_triples):
            ...
            for i in range(num_candidates):
                candidate_input = prompt(question, triples, candidates[:num_candidates-i])
                candidate_tokens = self.tokenizer.tokenize(self.task_instruction+candidate_input)
                if len(candidate_tokens) < self.max_length:
                    break
            inputs.append(candidate_input)
        
        assert len(inputs) == len(candidate_triples) # something wrong when obtaining selector inputs 

        return instructions, inputs 
    
    def get_selector_prompts_chat_format(self, instructions: List[List[str]], inputs: List[List[str]]):
        prompts = [instruction+ "\n" + text for instruction, text in zip(instructions, inputs)]
        return prompts
    
    def tokenizer_encode_chat_format(self, prompts: List[List[Dict[str, str]]]):
        batch_dict = self.tokenizer(prompts, max_length=self.max_length, padding=True, truncation=True, return_tensors='pt')
        tokenizer_outputs = {"input_ids": batch_dict["input_ids"], "attention_mask": batch_dict["attention_mask"]}
        return tokenizer_outputs
    
    def get_option_token_id_to_option_map(self, num_options: int):
        option_token_id_to_option_map = {}
        options = [str(i+1) for i in range(num_options)]
        for option in options:
            option_token_id_to_option_map[self.tokenizer.encode(option, add_special_tokens=False)[-1]] = option
            option_token_id_to_option_map[self.tokenizer.encode(f" {option}", add_special_tokens=False)[-1]] = option
        return option_token_id_to_option_map
    

class CoTTripleSelector(TripleSelector):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.selector = selector
        self.device = self.selector.device
        self.tokenizer = self.selector.tokenizer
        self.max_length = self.selector.max_length
        self.num_examplars = num_examplars
        self.adaptive_examplars = adaptive_examplars
        self.adaptive_examplars_retriever = adaptive_examplars_retriever
        self.use_sentences = use_sentences
        ...
        self.use_triple_filter = use_triple_filter
        if self.use_triple_filter:
            self.triple_retriever = self.load_triple_retriever(triple_filter_retriever, triple_filter_retriever_path)

        if triple_filter_reranker is not None:
            self.triple_reranker = self.load_triple_reranker(triple_filter_reranker, triple_filter_reranker_path)
        else:
            self.triple_reranker = None
        self.triple_filter_reranker_num = triple_filter_reranker_num
    ...
    def load_triple_retriever(self, retriever_name: str, retriever_path: Optional[str]=None) -> DenseRetriever:
        ...
        if "e5" in retriever_name.lower():
            ...
            print(f"loading E5 retriever from {retriever_path}...")
            retriever = BaseRetriever(retriever_name="E5Retriever", model_name_or_path=retriever_path)
        elif "bge" in retriever_name.lower():
            collator = BGECollator(tokenizer=tokenizer, query_maxlength=query_maxlength, doc_maxlength=doc_maxlength)
            print(f"loading BGE retriever from {retriever_path}...")
            retriever = BaseRetriever(retriever_name="BGERetriever", model_name_or_path=retriever_path)
        elif "contriever" in retriever_name.lower():
            collator = ContrieverCollator(tokenizer=tokenizer, query_maxlength=query_maxlength, doc_maxlength=doc_maxlength)
            print(f"loading Contriever retriever from {retriever_path}...")
            retriever = BaseRetriever(retriever_name="ContrieverRetriever", model_name_or_path=retriever_path)
        else:
            raise NotImplementedError(f"triple filter retriever \"{retriever_name}\" is not implemented!")
        ...
        dense_retriever = DenseRetriever(retriever=retriever, collator=collator)
    ...
    def load_triple_reranker(self, reranker_name: str, reranker_path: Optional[str] = None):
        ...
        reranker.to(self.device)
        reranker.eval()
        rerank_model = BGEKGChainReranker(reranker=reranker, collator=collator)
        return rerank_model
        ...
        **kwargs
        ...
        for triple in triples:
            ...
            if len(self.kg_generator.parse_triples_text(triple)) > 0:
                ...
                # print("Found Triple in id2triple: {} ==> {}. Score: {:.4f}\n".format(triple, id2triple[triple_id], max(scores)))
                id2score[triple_id] += 0.5 
            if "answer is:" in triple:
                ...
                for triple_id in sorted_triple_ids:
                    if answer.lower() in id2triple[triple_id]["text"].lower():
                        # print("Found Triple in id2triple: {} ==> {}.\n".format(triple, id2triple[triple_id]))
                        id2score[triple_id] += 0.5 
                        break
    ...
    def compute_triple_reranker_scores(
        self, 
        question: str, 
        reasoning_chains: List[List[Dict[str, Union[str, List]]]], 
        triples: List[Dict[str, Union[str, List]]],
        ...
        scores = self.triple_reranker.calculate_scores(
            questions=[question] * len(candidate_reasoning_chains_texts),
            chains=candidate_reasoning_chains_texts
        )
        scores = scores.reshape(len(reasoning_chains), -1)
    ...
    def filter_candidate_triples(
        ...
        queries_embeddings = self.triple_retriever.calculate_query_embeddings(queries=queries, max_length=256)

        # 计算triple的embedding
        triples_texts = [self.get_triple_text(triple) for triple in triples]
        triples_embeddings = self.triple_retriever.calculate_document_embeddings(documents=triples_texts, max_length=128)

        # 然后计算query-triple的相似度
        queries_triples_similarities = torch.matmul(queries_embeddings, triples_embeddings.T)

        # 如果有reranker的话, 使用reranker对triples进行重排序
        if self.triple_reranker:
            queries_triples_similarities = self.compute_triple_reranker_scores(question, reasoning_chains, triples, queries_triples_similarities)
    ...
    def one_cot_forward(
        ...
        question: str, 
        triples: List[dict], 
        reasoning_chains: List[Dict[str, Union[List, float, bool]]]=None,
        ranked_examplars_indices: Optional[List[int]]=None, 
        num_beams: Optional[int]=1, 
        max_num_chains: Optional[int]=None, 
        min_triple_prob: Optional[float]=1e-4, 
        **kwargs,
    ):
        
        chains, chains_scores, chains_finished = self.parse_reasoning_chains(reasoning_chains)

        # 得到selector模型的输入
        existing_triples = self.get_reasoning_chains_texts(chains)
        candidate_triples = [[self.get_triple_text(triple) for triple in triples] for _ in range(len(chains))] # 每一个chain的candidates都是一样的
        instructions, inputs = self.get_selector_inputs(
            question=question, 
            existing_triples=existing_triples, 
        ...
        prompts = self.selector.get_generator_prompts_chat_format(instructions, inputs)
        selector_inputs = self.selector.tokenizer_encode_chat_format(prompts)
        generated_token_ids, generated_token_logits = self.selector.generate(selector_inputs)

        option_tokens_list, option_tokens_probs = self.get_option_tokens_probs(
            generated_token_ids, generated_token_logits, len(triples) + 1 
        )
        ...
        for i in range(len(chains)):
            candidate_triples_for_current_chain = triples # = candidate_triples[i]
            # 如果当前路径已经结束或者没有任何的candidate triples的话，直接复制当前路径并结束
            if chains_finished[i] or len(candidate_triples_for_current_chain) == 0:
                new_chains.append(chains[i])
                new_chains_scores.append(chains_scores[i])
                new_chains_finished.append(True)
                continue
            ...
            for b in range(min(option_tokens_probs.shape[1], num_beams)):
                ...
                current_choice = option_tokens_list[topk_options_indices[i, b].item()]
                # 如果模型给的选项超过了当前可能的选项的时候跳过, 如果candidate_triples一共有两个，那么option就是["0", "1", "2"] 
                if current_choice != '0' and int(current_choice) > len(candidate_triples_for_current_chain):
                    continue
                new_chains_scores.append(chains_scores[i]*topk_options_probs[i, b].item())
                if current_choice == "0":
                    new_chains.append(chains[i])
                    ...
                    # -1是因为当choice为1的时候，实际上是candidate_triples_indices_for_current_chain中index为0的triple
                    new_chains.append(
                        chains[i]+[candidate_triples_for_current_chain[int(current_choice)-1]]
                    )
                    new_chains_finished.append(False)
    ...
    def cot_forward(self, question: str, num_beams: int=1, max_num_chains: int=None, min_triple_prob: Optional[float]=1e-4, **kwargs):

        ranked_prompt_indices = self.rank_examplars(question=question)
        ...
        for i in range(self.num_turns):
            ...
            # 首先把reasoning_chains中的信息分开
            chains, chains_scores, chains_finihsed = self.parse_reasoning_chains(reasoning_chains)
            if reasoning_chains is not None and np.sum(chains_finihsed) == len(reasoning_chains):
                break

            # 首先检索topk个document并生成对应的KG
            # queries = [question] if i == 0 else \
                # [self.get_triple_text(chain_item["triples"][-1]) for chain_item in reasoning_chains]
            queries = [question if len(chain) == 0 else question + "" + self.get_triple_text(chain[-1]) for chain in chains]
            ...
            # 然后根据triple_filter对每一条chain的candidate triples进行排序
            if self.use_triple_filter:
                candidate_triples_indices, candidate_triples_scores = self.filter_candidate_triples(
                    question=question, reasoning_chains=chains, triples=all_triples,
                    num_candidate_triples=self.maximum_possible_choices if i == 0 else self.num_candidate_triples
                )
            else:
                candidate_triples_indices = [list(range(min(len(all_triples), self.maximum_possible_choices)))for _ in range(len(chains))]
                candidate_triples_scores = None
            ...
            candidate_triples = candidate_triples[:self.maximum_possible_choices] # 限制最多可能的token数量

            reasoning_chains = self.one_cot_forward(
                question=question, 
                triples=candidate_triples, 
                reasoning_chains=reasoning_chains, 
                ranked_examplars_indices=ranked_prompt_indices,
                num_beams=num_beams, 
                max_num_chains=max_num_chains,
                min_triple_prob=min_triple_prob
            )
    ...
    def get_ircot_generator_inputs(self, question, triples, ranked_examplars_indices):
        ...
        if self.num_examplars > 0:
            ...
            for index in ranked_examplars_indices[:self.num_examplars]:
                ...
                # examplar = "Question: {}\nThought: {}".format(item["question"], thought)
    ...
    def get_ircot_generator_documents_inputs(self, question, documents, ranked_examplars_indices):
    ...
    # def ircot_generator_generate(self, instructions, inputs, reasoning_chains):
        
    #     # TODO 
    #     if self.selector.is_chat:
    #         prompts_chat_format = self.selector.get_generator_prompts_chat_format(instructions, inputs)
    #         selector_inputs = self.selector.tokenizer_encode_chat_format(prompts_chat_format)
    #         texts = ["Thought: " + ". ".join(chain) for chain in reasoning_chains]
    #         selector_inputs = append_texts_to_decoder_only_generator_inputs(
    #             tokenizer=self.tokenizer, inputs=selector_inputs, texts=texts # texts=["Thought:" + ". ".join(chain) for chain in reasoning_chains]
    #         )
    #     else:
    ...
    def ircot_generator_generate(self, instructions, inputs, reasoning_chains):
        ...
        texts = ["Thought: " + ". ".join(chain) for chain in reasoning_chains]
        return self.selector.generator_generate(instructions=instructions, inputs=inputs, current_generated_texts=texts)

    def one_ircot_forward(
        self, 
        question: str, 
        triples: List[dict], 
        ...
        instructions, inputs = self.get_ircot_generator_inputs(
            question=question, 
            triples=triples, 
            ranked_examplars_indices=ranked_examplars_indices
        )
        # instructions = instructions*len(reasoning_chains)
        # inputs = inputs*len(reasoning_chains)
        if documents is not None:
            document_instruction, document_input = self.get_ircot_generator_documents_inputs(
                question = question, 
                documents = documents, 
                ranked_examplars_indices=ranked_examplars_indices
            )
        ...
        generated_token_ids, _ = self.ircot_generator_generate(instructions, inputs, reasoning_chains)
        ...
        for chain, text in zip(reasoning_chains, generated_texts):
            ...
            if len(generated_triples) > 0:
                ...
                if len(text) > 0:
                    ...
                    chain.append(text)
        # print("generated_texts: {}\nreasoning_chains: {}\n".format(generated_texts, reasoning_chains))
        return reasoning_chains

    def ircot_forward(self, question: str, num_beams: int=1, **kwargs):
        ...
        triple_ids_to_scores, triple_ids_to_triple = {}, {}
        # for triple matching 
        embedding_indices_to_triple_ids, cached_triple_embeddings = {}, None

        #########下面的代码是为了得到每一个step top-ranked的document
        # top_ranked_docid_per_step = [] 
        ########################################
        ...
        for i in range(self.num_turns):
            ...
            # 然后根据triple_filter对每一条chain的candidate triples进行排序
            if self.use_triple_filter:
                chains = [[{"title": "", "text": triple} for triple in chain] for chain in reasoning_chains] # 为了适配filter_candidate_triples的输入格式
                candidate_triples_indices, candidate_triples_scores = self.filter_candidate_triples(
                    question=question, reasoning_chains=chains, triples=all_triples, num_candidate_triples=self.num_candidate_triples
                )
            else:
                chains = [[{"title": "", "text": triple} for triple in chain] for chain in reasoning_chains] # 为了适配filter_candidate_triples的输入格式
                candidate_triples_indices = [list(range(min(len(all_triples), self.num_candidate_triples)))for _ in range(len(chains))]
                candidate_triples_scores = None
            ...
            #########下面的代码是为了得到每一个step top-ranked的document
            # idx2score = {}
            # for indices, scores in zip(candidate_triples_indices, candidate_triples_scores):
            #     for idx, score in zip(indices, scores):
            #         idx2score[idx] = max(idx2score.get(idx, -999), score)
            # sorted_idx2score = sorted(idx2score.items(), key=lambda x: x[1], reverse=True)
            # top_ranked_docid_per_step.append([all_triples[idx]["reference"][0] for idx, _ in sorted_idx2score[:5]])
            ########################################

            # 生成下一步的reasoning chain
            reasoning_chains = self.one_ircot_forward(
                question=question, 
                triples=candidate_triples, 
                reasoning_chains=reasoning_chains, 
            ...
            # 用reasoning chain来对triple进行加权
            reasoning_chains_triples = [] 
            for chain in reasoning_chains:
                if len(chain) > i: # 只给新增加的triple
            ...
            # triple_ids_to_scores, triple_ids_to_triple, *others = self.update_reasoning_chains_triples_based_on_similarity(
            #     triple_ids_to_scores, triple_ids_to_triple, reasoning_chains_triples,
            #         embedding_indices_to_triple_ids, cached_triple_embeddings
            # )
            # if len(others) > 0:
            #     embedding_indices_to_triple_ids, cached_triple_embeddings = others
        
        docids_to_scores = self.get_docids_to_scores_from_triples(triple_ids_to_scores, triple_ids_to_triple)
        # from pdb import set_trace; set_trace()
        # return_reasoning_chains = [] 
        # for chain in reasoning_chains:
        #     new_chain = [{"title": "", "text": triple, } for triple in chain]
        #     return_reasoning_chains.append({"triples": chain, "score":0.0, "finished": False})
        ########################################
        return reasoning_chains, docids_to_scores
        # return reasoning_chains, docids_to_scores, top_ranked_docid_per_step
        #########下面的代码是为了计算检索triple的性能
        # triple_to_score = {} 
        # for triple_id, score in triple_ids_to_scores.items():
        #     triple_text = triple_ids_to_triple[triple_id]["text"]
        #     triple_to_score[triple_text] = max(triple_to_score.get(triple_text, -999), score)
        # return reasoning_chains, docids_to_scores, triple_to_score
        ########################################
    
    def ircot_forward_wo_selector(self, question: str, num_beams: int=1, **kwargs):
        ...
        for i in range(self.num_turns):
            ...
            # 生成下一步的reasoning chain (只选择排名第一的triple)
            for chain, triple_indices in zip(reasoning_chains, candidate_triples_indices):
                chain.append(all_triples[triple_indices[0]]["text"])
...
class CoTKGCorpusTripleSelector(CoTTripleSelector):
    ...
    def update_retrieved_triples(
        ...
        for one_retrieval_results in retrieved_triples:
            for triple in one_retrieval_results:
                triple_id = triple["triple_id"]
                triple_score = triple["score"]
                id2score[triple_id] = max(id2score.get(triple_id, -1e9), triple_score)
                id2triple[triple_id] = triple
    ...
    def ircot_forward(self, question: str, num_beams: int=1, **kwargs):
        ...
        assert num_beams == 1, "num_beams can only be set to 1 in CoTKGCorpusTripleSelector"
        ...
        for i in range(self.num_turns):
            ...
            queries = [question]*num_beams if i == 0 else \
                [question + " " + chain[-1] if len(chain)>0 else question for chain in reasoning_chains]
            retrieved_kg_triples = self.retriever(queries, topk=self.num_candidate_triples)
            triple_ids_to_scores, triple_ids_to_triple = self.update_retrieved_triples(
                triple_ids_to_scores, triple_ids_to_triple, retrieved_kg_triples
            )
            candidate_triples = self.get_triples(triple_ids_to_scores, triple_ids_to_triple)
        ...
        docids_to_scores = self.get_docids_to_scores_from_triples(triple_ids_to_scores, triple_ids_to_triple, max_num_docs=10)
        return reasoning_chains, docids_to_scores
...
if __name__ == "__main__":
    ...
    # selector = TripleSelector(
    #     tokenizer, model, examplar_type="hotpotqa", num_examplars=5, 
    #     use_sentences=True, use_triple_filter=True, verbose=False, use_title_in_triples=True
    # )
    selector = TextSelector(
        tokenizer, model, examplar_type="hotpotqa", num_examplars=5, 
        use_sentences=False, use_triple_filter=True, verbose=False, use_title_in_triples=False
    )
    
    # esr = [{'triples': [{'text': '<h1; r1; t1>', 'reference': ['0', 0]}], 'score': 0.4556674063205719, 'finished': False}, {'triples': [{'text': 'fourth sentence.', 'reference': ['1', 1]}], 'score': 0.16763067245483398, 'finished': False}, {'triples': [], 'score': 0.14793354272842407, 'finished': True}, {'triples': [{'text': 'second sentence.', 'reference': ['0', 1]}], 'score': 0.11521076411008835, 'finished': False}, {'triples': [{'text': 'fist sentence.', 'reference': ['0', 0]}], 'score': 0.06987886130809784, 'finished': False}]
    reasoning_chains = [] 
    for i in range(4):
        reasoning_chains = selector.forward(
            "this is a question", 
            documents=[
                {
                    "id": "0", "title": "title1", "text": "fist sentence. second sentence.",
                    "triples": [{"text": "<h1; r1; t1>", "sentence": 0}, {"text": "<h2; r2; t2>", "sentence": 1}]
                },
                {
                    "id": "1", "title": "title2", "text": "third sentence. fourth sentence.",
                    "triples": [{"text": "<h3; r3; t3>", "sentence": 0}, {"text": "<h4; r4; t4>", "sentence": 1}]
                },
            ],
            existing_reasoning_chains = reasoning_chains,
            max_num_chains=10, 
            # existing_reasoning_chains=[
            #     {
            #         "triples": [
            #             {"title": "title1", "text": "<h1; r1; t1>", "reference": ["0", 0]},
            #             # {"title": "title1", "text": "<h2; r2; t2>", "reference": ["0", 0]},
            #             # {"title": "title2", "text": "<h3; r3; t3>", "reference": ["0", 0]},
            #             {"title": "title2", "text": "<h4; r4; t4>", "reference": ["1", 0]},
            #         ], 
            #         "score": 0.8, 
            #         "finished": False
            #     }
            ...
            #     {
            #         "triples": [
            #             {"title": "title1", "text": "fist sentence.", "reference": ["0", 0]}
            #         ],
            #         "score": 0.8, 
            #         "finished": False
            #     }
            # ],
            reasoning_chains_type="sentence"
        )
        from pdb import set_trace; set_trace()
        a = 0 

    # reformulator = LLMQuerReformulator(tokenizer=tokenizer, reformulator=model)
    # reformulator.forward(
    #     question="Where is the headquarters of the company that produces the soft drink that the ICP traditionally sprays the audience with ?", 
    #     reasoning_chains=[
    #         [], 
    #         ["<Faygo; parent company; National Beverage Corporation>", "<FTFO; album art; alludes to ICP tradition of spraying audience with Faygo>"], 
    #         ["<Natalie Ravitz; previous role; staffer for United States Senators Barbara Boxer and Paul Wellstone>", "<Natalie Ravitz; previous occupation; Communications Director for the New York City Department of Education>"], 
    #         ["<Natalie Ravitz; previous role; staffer for United States Senators Barbara Boxer and Paul Wellstone>", "<Paul Wellstone; occupation; academic and politician>", "<Paul Wellstone; represented; Minnesota in the United States Senate>"]
    #     ]
    # )
